...
void Process::do_reference_of_type(RefType ref_type, int randref_num, pager::Pager *pager)
{
    if (ref_type == INIT_REF)
        do_next_reference(DELTA_UNDEF_, RANDREF_UNDEF_, pager);

    else if (ref_type == SEQ_REF)
        do_next_reference(DELTA_SEQ_, RANDREF_UNDEF_, pager);

    else if (ref_type == BACK_REF)
        do_next_reference(DELTA_BACK_, RANDREF_UNDEF_, pager);

    else if (ref_type == JMP_REF)
        do_next_reference(DELTA_JMP_, RANDREF_UNDEF_, pager);

    else if (ref_type == RAND_REF)
        do_next_reference(DELTA_UNDEF_, randref_num, pager);
}

void Process::do_next_reference(int delta, int randref_num, pager::Pager *pager)
{
